...
function Button({ children, className = "", disabled = false, variant = "solid", onClick }: ButtonProps) {
  const variantClass =
    variant === "outline"
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`${variantClass} inline-flex items-center justify-center rounded-2xl px-4 py-2 font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
    >
      {children}
    </button>
  );
...
function Badge({ children, className = "", variant = "solid" }: BadgeProps) {
  ...
  return (
    <span className={`${variantClass} inline-flex rounded-full px-3 py-1 text-xs font-bold ${className}`}>
      {children}
    </span>
  );
}
...
function Progress({ value, className = "" }: ProgressProps) {
  ...
  return (
    <div className={`overflow-hidden rounded-full bg-white/20 ${className}`}>
      <div className="h-full rounded-full bg-amber-300 transition-all" style={{ width: `${safeValue}%` }} />
    </div>
  );
}

function RobotMascot({ mood }: RobotMascotProps) {
  return (
    <motion.div
      className="relative mx-auto h-36 w-36"
      animate={{
        y: [0, -8, 0],
        rotate: mood === "correct" ? [0, -5, 5, 0] : mood === "wrong" ? [0, 4, -4, 0] : 0,
      }}
    ...
    >
      ...
      <div className="absolute left-5 top-6 h-24 w-28 rounded-3xl border-4 border-slate-800 bg-white shadow-xl">
        <div className="absolute -top-5 left-12 h-6 w-1 bg-slate-800" />
        <div className="absolute -top-8 left-10 h-5 w-5 rounded-full border-2 border-slate-800 bg-amber-300" />
      ...
      <motion.div
        className="absolute bottom-0 left-8 rounded-full bg-slate-900 px-3 py-1 text-xs font-bold text-white"
        animate={{ scale: mood === "correct" ? [1, 1.1, 1] : 1 }}
      >
        FTC BOT
      </motion.div>
    </motion.div>
...
export default function FTCJavaQuestQuiz() {
  ...
  const badge = useMemo<BadgeTitle>(() => {
    return [...badgeTitles].reverse().find((b) => score >= b.min) ?? badgeTitles[0];
  }, [score]);
        ...
        <div className="mx-auto max-w-5xl">
          <div className="grid gap-6 md:grid-cols-[1.1fr_.9fr] md:items-center">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              <Badge className="mb-4 bg-amber-300 text-slate-950">Friendly team check • Not a judgment</Badge>
              ...
              <p className="mt-4 max-w-2xl text-lg text-sky-100">
                A fun quiz adventure for new Java coders. Help the robot pass each mission by answering basics from Java syntax,
                output, comments, variables, data types, casting, operators, Strings, and booleans.
              </p>

              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                <Card className="border border-white/10 bg-white/10 text-white backdrop-blur">
                  <CardContent className="p-4">
                    <Code2 className="mb-2" /> 12 questions
                  </CardContent>
                </Card>
                <Card className="border border-white/10 bg-white/10 text-white backdrop-blur">
                  <CardContent className="p-4">
                    <Timer className="mb-2" /> 10–15 minutes
                  </CardContent>
                </Card>
                <Card className="border border-white/10 bg-white/10 text-white backdrop-blur">
                  <CardContent className="p-4">
                    <Trophy className="mb-2" /> Badge ending
                  </CardContent>
                </Card>
              </div>
              ...
              <Button
                ...
                className="mt-8 bg-amber-300 px-8 py-6 text-lg font-black text-slate-950 shadow-lg hover:bg-amber-200"
              >
                <Rocket className="mr-2" /> Start Mission
              </Button>

              <p className="mt-4 text-sm text-sky-200">
                Coach note: Let students play individually or in pairs. Celebrate explanations, not just scores.
              </p>
            </motion.div>
            ...
            <div className="relative">
              ...
              <motion.div
                className="mt-8 rounded-3xl bg-white/10 p-5 backdrop-blur"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              >
                <div className="flex items-center gap-2 font-bold">
                  <Sparkles className="text-amber-300" /> Mission Theme
                </div>
                <p className="mt-2 text-sky-100">
                  Every correct answer charges the robot battery. At the end, each student gets a fun title.
                </p>
              </motion.div>
            </div>
          </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-sky-900 p-6 text-white">
        <div className="mx-auto max-w-5xl">
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
            <div className="mx-auto mb-4 flex h-24 w-24 items-center justify-center rounded-full bg-amber-300 text-5xl shadow-xl">
              {badge.emoji}
            </div>
            <h1 className="text-5xl font-black">{badge.title}</h1>
            <p className="mt-3 text-xl text-sky-100">
              Score: {score} / {questions.length}
            </p>
            <p className="mt-2 text-sky-200">{badge.note}</p>
          </motion.div>

          <Card className="mt-8 border border-white/10 bg-white/95 shadow-2xl">
            <CardContent className="p-6">
              <h2 className="mb-4 text-2xl font-black text-slate-900">Mission Debrief</h2>
              <div className="grid gap-3">
                {answers.map((answerRecord, answerIndex) => (
                  <div key={`${answerRecord.topic}-${answerIndex}`} className="rounded-2xl border bg-slate-50 p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="text-sm font-bold text-indigo-700">{answerRecord.topic}</div>
                        <div className="font-semibold text-slate-900">{answerRecord.question}</div>
                        {!answerRecord.correct && (
                          <div className="mt-1 text-sm text-slate-600">Correct answer: {answerRecord.answer}</div>
                        )}
                      </div>
                    </div>
                  </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-sky-900 p-6 text-white">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <Badge className="bg-white text-slate-950">Score {score}</Badge>
        </div>
            ...
            <CardContent className="p-6 text-center">
              ...
              <div className="mt-6 rounded-2xl bg-white/10 p-4 text-left">
                <div className="flex items-center gap-2 font-black">
                  <Bot className="text-amber-300" /> Robot Battery
                </div>
              ...
              <Card className="border-0 bg-white shadow-2xl">
                <CardContent className="p-6 text-slate-900">
                  <div className="mb-3 flex flex-wrap gap-2">
                    <Badge className="bg-indigo-100 text-indigo-800">{q.level}</Badge>
                  ...
                  <div className="mt-5 grid gap-3">
                    {q.options.map((option, optionIndex) => {
                      const showCorrect = isAnswered && optionIndex === q.answer;
                      const showWrong = isAnswered && selected === optionIndex && optionIndex !== q.answer;
                      ...
                      return (
                        ...
                        >
                          <span className="mr-3 inline-flex h-7 w-7 items-center justify-center rounded-full bg-slate-900 text-sm text-white">
                  ...
                  {isAnswered && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className={`mt-5 rounded-2xl p-4 ${isCorrect ? "bg-green-50" : "bg-amber-50"}`}
                    >
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button disabled={!isAnswered} onClick={next} className="px-6">
